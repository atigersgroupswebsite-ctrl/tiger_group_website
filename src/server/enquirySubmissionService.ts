...
export interface EmployerEnquiryServerResult {
  status: number;
  data: {
    success: boolean;
    enquiryId?: string;
...
/**
 * Server-side authoritative handler for creating Employer Corporate Manpower Enquiries.
 * Validates allowlisted fields, ignores client-supplied status or admin fields,
 * inserts using server privileges, and returns database-generated identifiers.
 */
export async function submitEmployerEnquiryServerHandler(
  payload: any
): Promise<EmployerEnquiryServerResult> {
  if (!payload || typeof payload !== 'object') {
    return {
      status: 400,
      data: { success: false, error: 'Invalid request body.' }
  ...
  // 1. Sanitize & validate inputs (supports camelCase and snake_case)
  const trimmedCompany = typeof (payload.companyName || payload.company_name) === 'string'
    ? (payload.companyName || payload.company_name).trim()
    : '';
  const trimmedEmail = typeof payload.email === 'string'
    ? payload.email.trim().toLowerCase()
    : '';
  const rawPhone = typeof (payload.phone || payload.phoneNumber || payload.phone_number) === 'string'
    ? (payload.phone || payload.phoneNumber || payload.phone_number).trim()
    : '';
  const tenDigitPhone = extractTenDigitMobile(rawPhone);
  const trimmedAddress = typeof payload.address === 'string'
    ? payload.address.trim()
    : '';
  const trimmedDistrict = typeof payload.district === 'string'
    ? payload.district.trim()
    : '';
  const trimmedState = typeof payload.state === 'string'
    ? payload.state.trim()
    : '';
  const rawEmployees = payload.employeesRequired ?? payload.employees_required;
  const employeesRequired = Number(rawEmployees);
  const trimmedRole = typeof (payload.jobRole || payload.job_role) === 'string'
    ? (payload.jobRole || payload.job_role).trim()
    : '';
  const rawDescription = payload.description;
  const trimmedDescription = typeof rawDescription === 'string' && rawDescription.trim().length > 0
    ? rawDescription.trim()
    : null;
  ...
  if (!trimmedCompany || trimmedCompany.length < 2) {
    return {
      status: 400,
      data: { success: false, error: 'Company / Firm name is required (minimum 2 characters).' }
    };
  }
  ...
  if (!trimmedEmail || !EMAIL_REGEX.test(trimmedEmail)) {
    return {
      status: 400,
      data: { success: false, error: 'A valid official corporate email is required.' }
    };
  }

  if (!tenDigitPhone || !INDIAN_MOBILE_REGEX.test(tenDigitPhone)) {
    return {
      status: 400,
      data: { success: false, error: 'A valid 10-digit contact phone number is required (starting with 6, 7, 8, or 9).' }
    };
  }
  ...
  if (!trimmedAddress || trimmedAddress.length < 5) {
    return {
      status: 400,
      data: { success: false, error: 'Plant / Office address is required (minimum 5 characters).' }
    };
  }
  ...
  if (!trimmedDistrict) {
    return {
      status: 400,
      data: { success: false, error: 'District / City is required.' }
    };
  }
  ...
  if (!trimmedState) {
    return {
      status: 400,
      data: { success: false, error: 'State selection is required.' }
    };
  }
  ...
  if (isNaN(employeesRequired) || employeesRequired <= 0) {
    return {
      status: 400,
      data: { success: false, error: 'Number of employees required must be at least 1.' }
    };
  }
  ...
  if (!trimmedRole || trimmedRole.length < 2) {
    return {
      status: 400,
      data: { success: false, error: 'Job role / nature of work is required.' }
    };
  }

  // 2. Concurrency / In-Flight Duplicate Locking
  const normalizedPhoneWithPrefix = `+91${tenDigitPhone}`;
  const lockKey = `${trimmedEmail}:${trimmedCompany}`;
  if (activeEmployerSubmissions.has(lockKey)) {
    return {
      status: 429,
      data: { success: false, error: 'An employer requirement submission is already in progress for this firm. Please wait.' }
    };
  }
  ...
  try {
    ...
    // 3. Authoritative Insert
    // Status is strictly server-controlled: 'NEW'
    // enquiry_number is strictly database-generated by trigger trg_employer_enquiries_number
    const { data, error } = await supabaseServer
      .from('employer_enquiries')
      .insert({
        company_name: trimmedCompany,
        email: trimmedEmail,
        phone: normalizedPhoneWithPrefix,
        address: trimmedAddress,
        district: trimmedDistrict,
        state: trimmedState,
        employees_required: Math.floor(employeesRequired),
    ...
    if (error) {
      console.error('[submitEmployerEnquiryServerHandler] Supabase insert error:', error.message);
      return {
        status: 500,
        data: {
          success: false,
          error: error.message || 'Database error occurred while processing employer requirement.'
        }
      };
    }
    ...
    if (!data || !data.id) {
      return {
        ...
        data: {
          success: false,
          error: 'Employer enquiry was created but confirmation details could not be retrieved.'
        }
      };
    }
    ...
    console.error('[submitEmployerEnquiryServerHandler] Unexpected exception:', err);
    return {
      status: 500,
      data: {
        success: false,
        error: err?.message || 'Internal server error while submitting employer enquiry.'
      }
    };
  } finally {
