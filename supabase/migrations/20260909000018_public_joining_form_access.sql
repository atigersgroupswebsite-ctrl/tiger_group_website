-- ==============================================================================
-- Migration: 20260909000018_public_joining_form_access.sql
-- Description: Public Candidate Joining Form Access & Submission Architecture
-- Brand: A TIGER GLOBAL CAREER SOLUTION AND CONSULTANCY
--
-- Security & Operational Rules:
-- 1. Joining Form is PUBLIC: removes candidate authentication/OTP/magic-link requirements.
-- 2. Candidate provides an email address solely for receipt delivery and reference confirmation.
-- 3. Email is NOT an authentication factor and cannot be used to look up or modify prior submissions.
-- 4. Server authoritative: submit_joining_form_bundle creates a new record, generates unique JOIN-YYYY-XXXXXX,
--    associates documents, and locks to SUBMITTED state.
-- 5. Storage security: bucket 'candidate-documents' remains private; allows upload into 'candidates/',
--    while SELECT remains strictly restricted to active administrators.
-- 6. Admin authentication and authorization are completely preserved and untouched.
-- ==============================================================================

-- 1. Storage: Allow public candidate uploads into private candidate-documents bucket under candidates/
DROP POLICY IF EXISTS "Candidate can upload documents to candidate-documents" ON storage.objects;
...
-- Remove unneeded candidate read/update/delete policies that required auth.uid()
DROP POLICY IF EXISTS "Candidate can read own candidate-documents" ON storage.objects;
...
-- Ensure Admins have full access to storage objects
...
-- 2. Standalone Transactional RPC: Submit Joining Form Bundle (Public & Authenticated)
...
BEGIN
  ...
  -- 2. Extract & validate candidate email (syntactic validation only, not an auth credential)
  ...
  -- 6. Resolve photo & signature paths
  ...
  -- 8. Insert new standalone Joining Form record (immutable new submission)
  ...
  IF payload ? 'documents' AND jsonb_typeof(payload -> 'documents') = 'object' THEN
    FOR doc_key, doc_val IN SELECT * FROM jsonb_each(payload -> 'documents') LOOP
      doc_path := COALESCE(doc_val -> 'file' ->> 'storagePath', doc_val ->> 'storagePath');
      IF doc_path IS NOT NULL AND doc_path != '' THEN
        doc_name := COALESCE(doc_val -> 'file' ->> 'name', doc_val ->> 'name', doc_key);
        ...
        BEGIN
          INSERT INTO public.documents (
            ...
            form_id,
            target_app_id,
            doc_key::public.document_type,
            'SINGLE'::public.document_side,
            doc_path,
            doc_name,
            doc_mime,
            doc_size,
        ...
        EXCEPTION WHEN OTHERS THEN
          -- Document category enum fallback tolerance
          NULL;
        END;
      END IF;
    END LOOP;
